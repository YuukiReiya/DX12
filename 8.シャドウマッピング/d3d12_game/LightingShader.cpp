...
#pragma region 追加
		// シャドウマップ用リソース
		D3D12_GPU_DESCRIPTOR_HANDLE shadowMapSrv_, shadowSampler_;
#pragma endregion
	};
...
#pragma region 変更
		// リソースが増えたのでrootParams[5] -> rootParams[7]になりました
...
#pragma region 追加
		CD3DX12_DESCRIPTOR_RANGE shadowSrv, shadowSampler;
		shadowSrv.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 1);
		rootParams[5].InitAsDescriptorTable(1, &shadowSrv,
			D3D12_SHADER_VISIBILITY_PIXEL);

		shadowSampler.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER, 1, 1);
		rootParams[6].InitAsDescriptorTable(1, &shadowSampler,
			D3D12_SHADER_VISIBILITY_PIXEL);
#pragma endregion
...
#pragma region 追加
		// シャドウマップ設定
		impl_->commandList->SetGraphicsRootDescriptorTable(5, impl_->shadowMapSrv_);
		impl_->commandList->SetGraphicsRootDescriptorTable(6, impl_->shadowSampler_);
#pragma endregion
	}
...
#pragma region 追加
	void LightingShader::SetShadowMap(D3D12_GPU_DESCRIPTOR_HANDLE handle)
	{
		impl_->shadowMapSrv_ = handle;
	}
	void LightingShader::SetShadowMapSampler(D3D12_GPU_DESCRIPTOR_HANDLE handle)
	{
		impl_->shadowSampler_ = handle;
	}
#pragma endregion
}  // namespace dxapp
