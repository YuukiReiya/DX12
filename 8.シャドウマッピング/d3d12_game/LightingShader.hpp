...
#pragma region 追加
			DirectX::XMFLOAT4X4 shadowTransform;/*!< シャドウマップ用の行列 */
#pragma endregion
	...
	public:
		 ...
		 */
		void SetSrvDescriptorHeap(ID3D12DescriptorHeap* heap);
		 ...
		 * @param[in] offset ヒープ内でのオフセット位置
		 */
		void SetSamplerDescriptorHeap(ID3D12DescriptorHeap* heap);
		...
		/*!
		 * @brief カラーテクスチャをセットする
		 * @param[in] handle GPUハンドル
		 */
		void SetTexture(D3D12_GPU_DESCRIPTOR_HANDLE handle);

		/*!
		 * @brief カラーテクスチャのサンプラをセットする
		 * @param[in] handle GPUハンドル
		 */
		void SetTextureSampler(D3D12_GPU_DESCRIPTOR_HANDLE handle);
...
#pragma region 追加
	   ...
	   */
		...
		/*!
		 * @brief シャドウマップが塩ウするサンプラのセット
		 * @param[in] handle GPUハンドル
		 */
		void SetShadowMapSampler(D3D12_GPU_DESCRIPTOR_HANDLE handle);
#pragma endregion
	private:
		//! 内部実装クラス
		class Impl;
