...
std::unique_ptr<GeometoryMesh> GeometoryMesh::CreateQuad(
    ID3D12Device* device, DirectX::XMFLOAT3 pos, float w, float h,
    DirectX::XMFLOAT4 color) {
  std::vector<Vpcnt> v(4);
  // front
  v[0] = Vpcnt{{pos.x, pos.y - h, pos.z}, color, {0, 0, -1}, {0, 1}};
  v[1] = Vpcnt{{pos.x, pos.y, pos.z}, color, {0, 0, -1}, {0, 0}};
  v[2] = Vpcnt{{pos.x + w, pos.y, pos.z}, color, {0, 0, -1}, {1, 0}};
  v[3] = Vpcnt{{pos.x + w, pos.y - h, pos.z}, color, {0, 0, -1}, {1, 1}};


  // インデクス
  std::vector<std::uint32_t> i(6);
  // front
  i[0] = 0;
  i[1] = 1;
  i[2] = 2;
