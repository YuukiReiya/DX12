...
namespace dxapp {
//-------------------------------------------------------------------
// TextureViewManager::Impl
//-------------------------------------------------------------------
/*!
 * @brief TextureViewManagerの実装
 */
class TextureViewManager::Impl {
public:
  Impl() = default;
  ~Impl() = default;

  TextureView view(ID3D12Resource* resource) {
    const auto end = std::end(views_);
    const auto ret = views_.find(resource);
    if (ret == end) {
      return TextureView{};  // なかった
    }

    return views_[resource];
  }

  std::unordered_map<ID3D12Resource*, TextureView> views_;
};

//-------------------------------------------------------------------
// TextureViewManager
//-------------------------------------------------------------------
TextureViewManager::TextureViewManager() : impl_(new Impl()) {}

TextureViewManager::~TextureViewManager() {
  for (auto& v : impl_->views_) {
    v.second.handle.Free();
  }
}

TextureView TextureViewManager::CreateView(Device* device, ID3D12Resource* resource) {
  auto view = impl_->view(resource);
  if (view.resource != nullptr) {
    return view;
  }

  auto heap = device->viewDescriptorHeap();

  view.handle = heap->Allocate();
  view.resource = resource;

  auto srv = CD3DX12_CPU_DESCRIPTOR_HANDLE(view.handle.cpuHandle, view.handle.index,
                                           heap->incrementSize());
  ...
  // SRV生成
  device->device()->CreateShaderResourceView(resource, &desc, srv);

  impl_->views_[resource] = view;
  return view;
}

void TextureViewManager::DestroyView(TextureView* view) {
  auto it = impl_->views_.find(view->resource);
  if (it != std::end(impl_->views_)) {
    view->handle.Free();
    impl_->views_.erase(it);
  }
  view->resource = nullptr;
}

TextureView TextureViewManager::textureView(ID3D12Resource* resource) {
  return impl_->view(resource);
}
...
bool TextureManager::LoadWICTextureFromFile(Device* device,
  ...
  auto cl = device->CreateOnetimeGraphicsCommandList();
  {
    // VRAMへの転送コマンド発行
