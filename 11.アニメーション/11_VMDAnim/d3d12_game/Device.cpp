...
void Device::Initialize(
    ...
    std::array<uint32_t, D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES> descNums) {
  CreateDevice();
  ...
  // SRV/CBV/UAVヒープ
  {
    ...
    desc.NumDescriptors = descNums[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV];
    desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;

    viewDescriptorHeap_ = std::make_unique<DescriptorAllocator>();
    viewDescriptorHeap_->Initialize(device_.Get(), desc);
  }

  // SAMPLERヒープ
  {
    D3D12_DESCRIPTOR_HEAP_DESC desc{};
    desc.NumDescriptors = descNums[D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER];
    desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER;
    desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;

    samplerDescriptorHeap_ = std::make_unique<DescriptorAllocator>();
    samplerDescriptorHeap_->Initialize(device_.Get(), desc);
  }

  // RTVヒープ
  {
    D3D12_DESCRIPTOR_HEAP_DESC desc{};
    desc.NumDescriptors = descNums[D3D12_DESCRIPTOR_HEAP_TYPE_RTV];
    desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;

    rtvDescriptorHeap_ = std::make_unique<DescriptorAllocator>();
    rtvDescriptorHeap_->Initialize(device_.Get(), desc);
  }

  // DSVヒープ
  {
    D3D12_DESCRIPTOR_HEAP_DESC desc{};
    desc.NumDescriptors = descNums[D3D12_DESCRIPTOR_HEAP_TYPE_DSV];
    desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
    dsvDescriptorHeap_ = std::make_unique<DescriptorAllocator>();
    dsvDescriptorHeap_->Initialize(device_.Get(), desc);
  }
  ...
  // デプスバッファ
  {
    ...
    // デプスバッファをクリア（テクスチャを塗りつぶす）するときの設定
    dsClearValue_.Format = depthBufferFormat_;  // デプスバッファフォーマット
    dsClearValue_.DepthStencil.Depth = 1.0f;  // デプスは1.0
    dsClearValue_.DepthStencil.Stencil = 0;   // ステンシルは0
    ...
    device_->CreateCommittedResource(
        ...
        D3D12_RESOURCE_STATE_DEPTH_WRITE, &dsClearValue_,
    ...
    defaultDsvHandle_ = dsvDescriptorHeap_->Allocate();
    device_->CreateDepthStencilView(depthStencil_.Get(), &dsvDesc, defaultDsvHandle_.cpuHandle);
  }
}
...
void Device::CreateSwapChain(HWND hWnd, std::uint32_t width,
  ...
  for (std::uint32_t i = 0; i < backBufferSize_; i++) {
    ...
    defaultRtvHandle_[i] =  rtvDescriptorHeap_->Allocate();
    CD3DX12_CPU_DESCRIPTOR_HANDLE rtvDescriptor(
      defaultRtvHandle_[i].cpuHandle, defaultRtvHandle_[i].index,
      rtvDescriptorHeap_->incrementSize());
...
Device::CreateOnetimeGraphicsCommandList() {
...
void Device::Present() {
  // コマンドを実行する
  commandQueue_->ExecuteCommandLists(commandLists_.size(),
                                     commandLists_.data());
...
void Device::PushCommandList(ID3D12CommandList* commandList)
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE Device::currentRenderTargetView() const {
  return CD3DX12_CPU_DESCRIPTOR_HANDLE(
      defaultRtvHandle_[backBufferIndex_].cpuHandle,
      defaultRtvHandle_[backBufferIndex_].index,
      rtvDescriptorHeap_->incrementSize());
}

CD3DX12_CPU_DESCRIPTOR_HANDLE Device::depthStencilView() const {
  return CD3DX12_CPU_DESCRIPTOR_HANDLE(
    defaultDsvHandle_.cpuHandle,
    defaultDsvHandle_.index,
    dsvDescriptorHeap_->incrementSize());
}
}  // namespace dxapp
