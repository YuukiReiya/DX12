...
// DirectXTK ÇÃ KeyboardÉNÉâÉXÇ©ÇÁUWPånÇÃèàóùÇçÌèú
...
class Keyboard::Impl {
 public:
  Impl(Keyboard* owner) : mState{}, mOwner(owner) {
    if (s_keyboard) {
  ...
  void Reset() { memset(&mState, 0, sizeof(State)); }
...
void Keyboard::Reset() { pImpl->Reset(); }

bool Keyboard::IsConnected() const { return pImpl->IsConnected(); }
