...
namespace dxapp {
#pragma region BasicShader::Impl
/*!
 * @brief BasicShaderの実装
 */
...
// BasicShader
//-------------------------------------------------------------------
BasicShader::BasicShader() : impl_(new Impl) {}

BasicShader::~BasicShader() {}

void BasicShader::Initialize(ID3D12Device* device) {
  impl_->CreateShader();
...
void BasicShader::Begin(ID3D12GraphicsCommandList* commandList) {
...
void BasicShader::SetCBufferDescriptorHeap(ID3D12DescriptorHeap* heap,
                                           const int offset) {
...
void BasicShader::SetSrvDescriptorHeap(ID3D12DescriptorHeap* heap,
                                       const int offset) {
  impl_->cbvSrvHeap_ = heap;
  impl_->srv_ =
      CD3DX12_GPU_DESCRIPTOR_HANDLE(heap->GetGPUDescriptorHandleForHeapStart(),
...
void BasicShader::SetSamplerDescriptorHeap(ID3D12DescriptorHeap* heap,
                                           const int offset) {
...
#pragma region BasicNonTexShader::Impl
/*!
 * @brief BasicNonTexShaderの実装
 */
class BasicNonTexShader::Impl {
 public:
  /*!
   * @brief コンストラクタ
   */
  ...
  // コマンドリスト積み込みに使うデスクリプタのキャッシュ
  // なんかもうちょっとスマートにやりたい
  ID3D12DescriptorHeap *cbvHeap_{};
  CD3DX12_GPU_DESCRIPTOR_HANDLE cbv_;
};

void BasicNonTexShader::Impl::CreateShader() {
  {
    Microsoft::WRL::ComPtr<ID3DBlob> error;
    utility::CompileShaderFromFile(L"Shaders/BasicNonTexVS.hlsl", L"vs_6_0", vs_,
                                   error);
    utility::CompileShaderFromFile(L"Shaders/BasicNonTexPS.hlsl", L"ps_6_0", ps_,
                                   error);
  }
}

void BasicNonTexShader::Impl::CreatRootSignature(ID3D12Device* device) {
...
void BasicNonTexShader::Impl::CreatePipelineState(ID3D12Device* device) {
  D3D12_GRAPHICS_PIPELINE_STATE_DESC psoDesc{};
  // シェーダーのセット
  psoDesc.VS = CD3DX12_SHADER_BYTECODE(vs_.Get());
  psoDesc.PS = CD3DX12_SHADER_BYTECODE(ps_.Get());
...
BasicNonTexShader::BasicNonTexShader() : impl_(new Impl) {}
BasicNonTexShader::~BasicNonTexShader() {}

void BasicNonTexShader::Initialize(ID3D12Device* device) 
{
  impl_->CreateShader();
  impl_->CreatRootSignature(device);
  impl_->CreatePipelineState(device);
...
void BasicNonTexShader::Terminate() {}
void BasicNonTexShader::Begin(ID3D12GraphicsCommandList* commandList) {
  assert(impl_->commandList == nullptr);
  impl_->commandList = commandList;
...
void BasicNonTexShader::End() {
  assert(impl_->commandList);
  impl_->commandList = nullptr;
}
void BasicNonTexShader::Apply() {
  ID3D12DescriptorHeap* heaps[] = {impl_->cbvHeap_,};
...
void BasicNonTexShader::SetCBufferDescriptorHeap(ID3D12DescriptorHeap* heap,
                                                 const int offset) {
  impl_->cbvHeap_ = heap;
  impl_->cbv_ =
      CD3DX12_GPU_DESCRIPTOR_HANDLE(heap->GetGPUDescriptorHandleForHeapStart(),
